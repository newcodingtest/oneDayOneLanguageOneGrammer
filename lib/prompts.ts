...
export const OLD_VERB_PROMPT = (years: number, month: number, day: number) => {
  ...
  const VERB_TOPIC_CURRICULUM: Record<number, string> = {
  1: "일상 시작/준비",
  2: "일상 종료/정리",
  3: "약속과 일정",
  4: "회사 업무 진행",
 ...
 const targetTopic = VERB_TOPIC_CURRICULUM[day] || "일상 회화";

return `당신은 한국인을 위한 영어 학습 콘텐츠 제작자다.

오늘 반드시 다뤄야 할 주제는 오직 "${targetTopic}" 이다.
"${targetTopic}" 상황에서 실제로 자주 쓰이는 구동사만 선택하라.
...
- 총 5개의 구동사를 생성한다
- 난이도: 초중급
- "${targetTopic}" 맥락에서 실제 회화, 비즈니스, 콘텐츠, SNS에서 자주 쓰이는 표현만 사용
...
- 감정 태그 금지
- 반드시 상황 기반으로 작성
- category는 반드시 "${targetTopic}" 또는 "${targetTopic}"과 직접적으로 연결되는 짧은 상황명으로 작성한다
...
세부 규칙:
1. items는 반드시 "${targetTopic}" 상황 5개 생성
...
const targetTopic = curriculum[day] || "일상 회화에서 자주 쓰는 슬랭";

return `너는 영어 교육용 콘텐츠를 만드는 원어민 감각의 영어 선생님이다.
목표는 한국인 학습자가 실제 원어민이 자주 쓰는 slang 표현을 자연스럽게 익히도록 돕는 것이다.

오늘 반드시 다뤄야 할 주제는 오직 "${targetTopic}" 이다.
오늘 생성하는 slang 2개는 반드시 "${targetTopic}" 상황에서 실제로 자주 보이거나 들리는 표현이어야 한다.
...
2. 반드시 "${targetTopic}" 상황에 어울리는 표현만 만든다.
3. 실제 회화, SNS, 유튜브, 드라마, 욕설 등에서 볼 수 있는 표현 위주로 만든다.
...
14. category 는 "${targetTopic}"과 직접 연결되는 짧고 직관적인 상황명으로 작성한다.
...
export const MISTAKE_PROMPT = (
  year: number,
  month: number,
  day: number
) => {

  const MISTAKE_TOPICS: Record<number, string> = {
  1: "직역해서 틀리는 영어 (한국식 사고)",
  2: "외국인 앞에서 자주 하는 실수",
  3: "친구랑 대화하다가 틀리는 표현",
  4: "연애/썸에서 자주 틀리는 영어",
  5: "회사에서 자주 하는 영어 실수",
  ...
  12: "부탁할 때 무례해지는 영어",
  13: "공손하게 말하려다 망하는 영어",
  14: "쿨하게 말하려다 어색해지는 영어",
  15: "원어민은 절대 안 쓰는 콩글리시",
  16: "뜻은 맞는데 어색한 영어",
  17: "한국인만 쓰는 이상한 표현",
  18: "헷갈리는 비슷한 표현",
  19: "너무 교과서 같은 영어",
  20: "너무 직설적으로 들리는 영어",
  21: "너무 약하게 들리는 영어",
  22: "톤이 이상해지는 영어 (무례/오해)",
  23: "자주 틀리는 짧은 문장",
  24: "외국인이 이해 못하는 영어",
  25: "상황에 안 맞는 영어",
  26: "자연스럽게 말하려다 틀리는 영어",
  27: "SNS에서 자주 틀리는 영어",
  28: "밈처럼 쓰는 영어 실수",
  29: "한국인이 많이 오해하는 영어",
  30: "실제로 창피했던 영어 실수 상황"
};

   const targetTopic = MISTAKE_TOPICS[day] || "직역해서 틀리는 영어 (한국식 사고)";

  return `너는 한국인 영어 학습자를 위한 콘텐츠를 만드는 원어민 감각의 영어 선생님이다.

오늘의 주제: "${targetTopic}"

목표:
한국인이 실제로 많이 공감하고 "나도 이거 틀렸는데" 라고 느낄 수 있는 영어 실수를 만들어라.

중요:
이 콘텐츠는 SNS에서 공유될 수 있어야 한다.
따라서 재미, 공감, 현실감이 반드시 포함되어야 한다.

반드시 JSON만 반환해라.

조건:
1. 총 3개의 표현 생성
2. 실제 한국인이 자주 하는 실수여야 한다
3. 틀린 표현은 "진짜 한국인이 쓸 것 같은 문장"으로 작성
4. reason은 "왜 한국인이 이렇게 말하는지" 중심으로 설명
5. nuance는 원어민이 느끼는 어색함 / 느낌 차이 설명
6. example은 현실 대화처럼 작성 (친구, 회사, 여행 상황 등)
7. tip은 짧고 기억에 남게
